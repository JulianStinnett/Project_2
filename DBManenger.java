...
public class DBManager {
    public String DBLocation = "dronedata.sqlite"; // For running from an IDE w/ DB
    located in local project folder.
    // For running in IDE but accessing full path to DB in Tomcat website (NOTE:
    May need to adjust 8.5).
    //public String DBLocation = "C:\\Program Files\\Apache Software Foundation\\
    Tomcat 8.5\\webapps\\dronereconportal\\db\\dronedata.sqlite";
    ...
    public ArrayList<AreaGridTile> readAreaGridTiles(String sAreaID){
        ...
        try {
            c.setAutoCommit(false);
            stmt = c.createStatement();
            ResultSet rs = stmt.executeQuery( "SELECT * FROM AreaGridTiles WHERE
                    area_id = '" + sAreaID + "';" );
            while ( rs.next() ) {
